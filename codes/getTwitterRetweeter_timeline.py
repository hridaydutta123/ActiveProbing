...
for users in userList:
	for status in tweepy.Cursor(api.user_timeline, users).items():
		...
		for rts in retweeters:
			print "retweeter " + str(rts)
			for status in tweepy.Cursor(api.user_timeline, user_id=rts).items():
				rtes_timeline.append(status._json)

	
		with open("../data/" + str(users) + "_retweeter_timeline.csv","a") as fp:
			fp.write(str(datetime.datetime.now()) + "," + str(users) + "," + str(status._json['id']) + "," + str(rtes_timeline) + "\n\n")
